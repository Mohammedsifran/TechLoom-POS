...
// Order Lifecycle (Cancellation and Refunds)
app.post('/api/orders/:id/cancel', async (req, res) => {
  ...
  try {
    ...
    if (order.status !== 'Reserved' && order.status !== 'Pending' && order.status !== 'Paid') {
      throw new Error(`Cannot cancel/refund order in status: ${order.status}`);
    }
    ...
    await connection.query('UPDATE orders SET status = ? WHERE id = ?', [newStatus, req.params.id]);
    await connection.commit();

    res.json({ success: true, message: `Order ${newStatus.toLowerCase()}, stock restored.` });
  } catch (error) {
    await connection.rollback();
    res.status(400).json({ error: error.message });
  } finally {
